 ...
 */
class userInfo {
    //  根据用户获取下一等级还需要消费的金额，名称(性能调优标识)
    static async getVipLevelInfo(ctx, params) {
        return ctx.$get(ctx.nettyUrl + ctx.serverPortUrl.memberService + ctx.optimizeFlag + '/vip-level-upgs/next-level/info', params).then((res) => {
            return res
        });
    }
    ...
    static async getVipInfoValue(ctx, params) {
        return ctx.$get(ctx.nettyUrl + ctx.serverPortUrl.memberService + ctx.optimizeFlag + '/vip-info-hds/index-info/usr-id', params).then((res) => {
            return res
        });
    }
    ...
    static async getUserInfoValue(ctx, params) {
        ...
        return ctx.$get(ctx.nettyUrl + ctx.serverPortUrl.memberService + ctx.optimizeFlag + url, params, ctx.headers).then((res) => {
            return res
        });
    }

    //  修改个人资料（会员整合新增vipInfoHdId）
    static async updateUserInfoValue(ctx, params) {
        let url = '/vip-info-hds/usr-id/' + params.vipInfoHdId + '?companyId=' + params.companyId
