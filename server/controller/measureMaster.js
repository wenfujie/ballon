...
class measureMasterController {
    ...
    // 获取量体类别(默认量体类别)
    static async getDefaultMeasureType(ctx) {
    ...
    // 根据商品id获取商品类别及量体项信息
    static async getGoodsMsrInfo(ctx) {
        let res = await measureMaster.getGoodsMsrInfo(ctx, ctx.params);
        ctx.body = res.data
    }

    // 获取自动放量
    static async getReleaseValue(ctx) {
