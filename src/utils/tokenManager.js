...
const tokenManager = {
    ...
    async getCilentToken() {
        if(!!Storage.get('TOKENINFO')) {
            // console.log('浏览器缓存中的token',Storage.get('TOKENINFO').access_token)
            ...
            let res = await axios.post('token/cilentToken',data)
            let tokenInfo = res
            Storage.set('TOKENINFO',tokenInfo,6660)  // 设置token存储时间为1小时52分钟
