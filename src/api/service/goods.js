 ...
 * @returns {*}
 */
export const isShelves = (config) => {
    return http.get('goods/isShelves', config);
}
 ...
 * @returns {*}
 */
export const getGoodsClassTree = (config) => {
    return http.get('goods/goodsClassTree', config);
}
 ...
 * var param = {
    companyId:公司id,
    usrId: 用户id,
    count: 每个图片显示图片数量-integer,
    partIds: 商品ids-Array[long]
    }
 * @returns {*}
 */
export const getGoodsCoverList = (config) => {
    return http.post('goods/goodsCoverList', config);
}
