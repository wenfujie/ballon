 ...
 *  param = {
    ...
    usrId: 用户id,
    pageNum: 0,
    reasonTypeCode: 'D_CANCEL',  // (退单类型)
    effFlag: 1,  // (是否启用0否，1是)
}
 * @returns {*}
 */
export const getReasonsList = config => {
    return http.get("order/reasonList", config);
};
 ...
 * @returns {*}
 */
export const getOrderNum = config => {
    return http.get("order/orderNum", config);
};
...
export const clearCache = config => {
    return http.get("order/clearCache", config);
};
