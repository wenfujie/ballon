 ...
 * @LastEditTime: 2019-04-22 17:22:59
 */
...
export default {
    ...
    async pay({ dispatch, state }, params) {
        ...
        wxPay(billCode, price).then(config => {
            let goodCodes = [];
            params.order.goodsList.forEach(good => {
                goodCodes.push(good.goodsCode);
            });
            payment.miniProgramPay(config, params.successUrl, params.failUrl);
        });
    }
};
